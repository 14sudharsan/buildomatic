...
import com.jaspersoft.jasperserver.war.common.SessionAttributeManager
import com.jaspersoft.ji.adhoc.service.AdhocEngineServiceImpl
import com.jaspersoft.ji.adhoc.action.AdhocActionConstants
...
class AdhocModChecker implements RequestMonitor {
	...
	def getState(req) {
		def iEngine = getSessionAttr(req, AdhocActionConstants.ENGINE)
		iEngine.state
	}
	
	// look up last saved view
	def getSavedView(req) {
		def currentView = getSessionAttr(req, AdhocActionConstants.ADHOC_REPORT)
		def savedView 
		if (currentView) {
