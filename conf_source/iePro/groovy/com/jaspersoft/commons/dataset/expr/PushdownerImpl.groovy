...
class PushdownerImpl extends ExpressionTransformerImpl implements Pushdowner {
    ...
    Expression get(Operator op) {
        ...
        // pass on parens and set "not" if applicable
        if (result instanceof Operator) {
            result.paren = op.paren
            result.not = op.not
        }
        result
    }
    ...
    Expression handleGroup(Expression groupExpr) {
        // To be consistency with GroupCalculation we need to add paren true
        // to groupExpr. Only for CountDistinct
        if (groupExpr instanceof Operator) {
            groupExpr = ((Operator) groupExpr).setParen(true);
        }
